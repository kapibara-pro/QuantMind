...
标准化层使用 `qm_ths_standardized_snapshots` 保存稳定的代码、名称、排名、价格、涨跌幅、成交量、估值、涨停梯队、情绪和板块字段；未纳入统一字段的上游字段进入 `extra` JSONB。每个数据集同时创建 `v_ths_std_<dataset>` 过滤视图，管理台预览读取标准化表并动态返回列定义，原始表仅用于追溯。部署已有历史快照时执行 `python backend/scripts/ths_standardize_snapshots.py`，会幂等创建标准化层并回填已有原始记录。
...
管理员也可以在「个人中心 → 数据平台」的「同花顺快照」配置卡片中填写 API Key、服务地址、股票/指数范围和定时采集开关。配置写入 `config/runtime.env`，页面只显示脱敏值；修改定时开关后需重启 Celery beat 使新的调度配置生效。
...
- `backend/scripts/ths_standardize_snapshots.py`：创建并回填标准化快照层。
